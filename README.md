...
Annopick 的 ZCode 插件，集成 WeKnora 知识库技能、前端开发/验收智能体、ZAI MCP 服务器与配置注入指令。面向前端工程化开发与端到端验收场景。
...
| 组件 | 文件 | 说明 |
|------|------|------|
| **Skill** | `skills/weknora` | 通过 WeKnora REST API 导入文档、检索知识库、发起 RAG/Agent 问答 |
| **Agent** | `agents/frontend-developer.md` | 前端开发智能体（Vue3 + TS + Element Plus），承担编码/详细设计/单测文档 |
| **Agent** | `agents/frontend-acceptance.md` | 前端 E2E 验收智能体，基于 Playwright 做功能验证与截图取证 |
| **Command** | `commands/inject-mcp-token.md` | 兜底：把 ZAI MCP token 注入 `.mcp.json` |
| **Command** | `commands/inject-agent-model.md` | 为前端 agent 注入 `model:` 字段（默认 Kimi K3） |
| **MCP** | `.mcp.json` | 5 个 MCP 服务器（见下表） |
...
4 个 `zai-*` 服务器的鉴权统一由插件清单声明的 **userConfig** 字段 `zai_api_token` 驱动，ZCode 加载插件时自动把 `.mcp.json` 里的 `${user_config.zai_api_token}` 替换为用户配置的真实值并注入 `env`/`headers`。
...
两个前端 agent 的 `model:` 字段是占位符，需注入真实模型值。默认 Kimi K3，执行指令：
...
| `/inject-agent-model` | 为 `frontend-developer` / `frontend-acceptance` 注入 `model:` 字段 |
