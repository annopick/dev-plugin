...
Annopick 的 ZCode 插件，集成 WeKnora 知识库技能、Ant Design 全栈知识技能（antd 组件 / ProComponents / Umi Max / AntV 可视化 / AI 原生组件）、前端开发/验收智能体（Vue3 与 React 两套技术栈）、ZAI MCP 服务器与配置注入指令。面向前端工程化开发与端到端验收场景。
...
├── .mcp.json                    # MCP 服务声明（8 个 server）
├── commands/                    # 斜杠指令
│   ├── inject-mcp-token.md
│   ├── inject-agent-model.md
│   └── plugin-version-release.md
...
│   │   ├── SKILL.md
│   │   └── API_REFERENCE.md
│   ├── antd/                    # Ant Design 组件知识技能（antd CLI MCP/CLI）
│   │   └── SKILL.md
│   ├── antd-pro/                # Ant Design Pro + ProComponents 知识技能
│   │   └── SKILL.md
│   ├── umi/                     # Umi Max 4 框架知识技能
│   │   └── SKILL.md
│   ├── antv/                    # AntV 可视化知识技能（MCP 图表生成 + G2/G6/X6）
│   │   └── SKILL.md
│   ├── x-components/            # @ant-design/x UI 组件（官方 x-skill）
│   │   ├── SKILL.md
│   │   └── reference/
│   ├── use-x-chat/              # useXChat Hook（官方 x-skill）
│   │   ├── SKILL.md
│   │   └── reference/
│   ├── x-request/               # XRequest 流式请求（官方 x-skill）
│   │   ├── SKILL.md
│   │   └── reference/
│   ├── x-chat-provider/         # 自定义 Chat Provider（官方 x-skill）
│   │   ├── SKILL.md
│   │   └── reference/
│   ├── x-markdown/              # XMarkdown 流式渲染（官方 x-skill）
│   │   ├── SKILL.md
│   │   └── reference/
...
| **Skill** | `skills/antd` | 通过 antd CLI MCP/CLI 离线查询组件 API、文档、Demo、Token 等 |
| **Skill** | `skills/antd-pro` | Ant Design Pro + ProComponents 知识——ProTable/ProForm/ProLayout 全组件 API、CRUD 模板、配置参考 |
...
| **Command** | `commands/plugin-version-release.md` | 插件版本发布：升级版本号→修订 CHANGELOG/README→commit→push+tag |
| **MCP** | `.mcp.json` | 8 个 MCP 服务器（见下表） |
...
| `antv-chart` | stdio | AntV 图表生成（26 个 generate_* 工具，返回图片 URL） | 无 |

4 个 `zai-*` 服务器的鉴权由 userConfig 字段 `zai_api_token` 驱动；`weknora` 服务器由 `weknora_base_url` 与 `weknora_api_key` 驱动。ZCode 加载插件时自动把 `.mcp.json` 里的 `${user_config.*}` 占位符替换为用户配置的真实值并注入 `env`/`headers`。`playwright`、`antd` 与 `antv-chart` 无需鉴权，开箱即用。
...
| `/plugin-version-release` | 插件版本发布：升级三处清单版本号→修订 CHANGELOG/README→commit→push+tag |
