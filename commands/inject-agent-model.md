...
插件内两个前端 agent（`frontend-developer`、`frontend-acceptance`）的 frontmatter `model:` 字段是占位符，需在安装后写入真实的 `custom:<provider>:<model-id>` 值才能按指定模型运行。
...
2. 脚本会自动：从 `~/.zcode/v2/config.json` 校验 provider/model 真实存在（不存在则报错并列出可用项）；探测插件安装目录最新版本；对两个 agent 文件——已有 `model:` 行则原地替换、没有则在 frontmatter 结束 `---` 前插入；写前备份为 `<file>.bak`；幂等（已是目标值则跳过）。
...
把脚本的 stdout 原样转达给用户，明确：两个 agent 各自更新成了什么 model 值、备份文件路径、以及如果想换模型如何再次执行。
